...
def parse_args():
	...
	parser.add_argument('--with_rgb', action='store_true', default=False,
						help='Whether to use RGB information [default: False]')
	parser.add_argument('--step_size', type=int, default=200,
						help='Decay step for lr decay [default: every 200 epochs]')
...
def main(args):
	...
	TRAIN_DATASET = S3DISDataset(root, split='train', with_rgb=args.with_rgb, test_area=args.test_area,
								 block_points=NUM_POINT)
	print("start loading test data ...")
	TEST_DATASET = S3DISDataset(root, split='test', with_rgb=args.with_rgb, test_area=args.test_area,
								block_points=NUM_POINT)
	print("start loading whole scene validation data ...")
	TEST_DATASET_WHOLE_SCENE = S3DISDatasetWholeScene(root, split='test', with_rgb=args.with_rgb,
													  test_area=args.test_area, block_points=NUM_POINT)
	...
	for epoch in range(start_epoch, args.epoch):
		...
		total_correct, total_seen, total_sum = 0, 0, 0

		for i, data in tqdm(enumerate(trainDataLoader), total=num_batches, smoothing=0.9):
