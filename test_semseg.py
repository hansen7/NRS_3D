"""
Author: Benny and Hanchen
Date: Nov 2019
"""
#  Copyright (c) 2020. Hanchen Wang, hw501@cam.ac.uk

import sys, os, torch, logging, importlib, argparse, provider, numpy as np
...
def parse_args():
	...
	parser.add_argument('--batch_size', type=int, default=12,
						help='batch size in testing [default: 12]')
	parser.add_argument('--gpu', type=str, default='0',
						help='specify gpu device')
	parser.add_argument('--num_point', type=int, default=8192,
						help='Point Number [default: 8192]')
	parser.add_argument('--log_dir', type=str, default='pointnet2_ssg',
...
def main(args):
	...
	TEST_DATASET_WHOLE_SCENE = ScannetDatasetWholeScene_evaluation(
		root, split='test', with_rgb=WITH_RGB, test_area=args.test_area, block_points=NUM_POINT)
	...
	with torch.no_grad():
		...
		for batch_idx in range(num_batches):
			...
			# (num of points in entire scenes, num of classes)

			# nums_vote -> 3
			for _ in tqdm(range(args.num_votes), total=args.num_votes):
				scene_data, scene_label, scene_smpw, scene_point_index = TEST_DATASET_WHOLE_SCENE[batch_idx]
				num_blocks = scene_data.shape[0]  # blocks of an entire room ???
